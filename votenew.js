...
async function loadPositions() {
  ...
  const { data: positions } = await supabase.from('position').select('*')
...
window.submitVotes = async function () {
  ...
  for (let position_id in vote) {

    const candidate_id = vote[position_id]
  ...
  window.location.href = "resultnew.html"
}